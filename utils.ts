...
export async function setupEnvironment(): Promise<EnvironmentData> {
  ...
  rstestPath = path.resolve(workspace, 'rstest');
  cwd = process.cwd();
  env = {
    ...process.env,
    CI: 'true',
  ...
  return { root, workspace, rstestPath, cwd, env };
}
...
let rstestPackages: Packages | null = null;

async function getRstestPackage() {
  const packages = rstestPackages || (await getPackages(rstestPath));

  rstestPackages = packages;
...
export async function runInRepo(options: RunOptions & RepoOptions) {
  ...
  const rstestPackage = await getRstestPackage();
  const packages = [...rstestPackage.packages];
  if (options.release) {
...
export async function setupRstestRepo(options: Partial<RepoOptions>) {
  const repo = options.repo || 'web-infra-dev/rstest';
  await setupRepo({
    repo,
    dir: rstestPath,
    branch: 'main',
...
export async function buildRstest({ verify = false }) {
  cd(rstestPath);
...
export async function bisectRstest(
  good: string,
  runSuite: () => Promise<Error | void>,
) {
  // sometimes rstest build modifies files in git, e.g. LICENSE.md
...
export function parseRstestMajor(rstestPath: string): number {
  const content = fs.readFileSync(
    path.join(rstestPath, 'packages', 'core', 'package.json'),
    'utf-8',
  );
